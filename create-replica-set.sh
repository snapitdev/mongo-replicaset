...
function copyFilesToContainer {
  ...
  docker cp ./bikes.js $1:/data/admin
}
...
function add_bikesdb_test {
  ...
  sleep 1
  docker exec -i mongoNode1 bash -c 'mongo -u $MONGO_USER_ADMIN -p $MONGO_PASS_ADMIN --authenticationDatabase "admin" < /data/admin/bikes.js'
}
